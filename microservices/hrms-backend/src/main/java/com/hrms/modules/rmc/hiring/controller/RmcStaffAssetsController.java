...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/rmc/staff-assets")
public class RmcStaffAssetsController {
	@Autowired
	private RmcStaffAssestsService staffAssetsService;
	...
	public ResponseEntity<List<RmcStaffAssests>> findByStaff(@PathVariable String staffId) {
		List<RmcStaffAssests> assetsList = staffAssetsService.findByStaff(Long.parseLong(decoder(staffId)));
	...
	public ResponseEntity<String> updateStaffAsset(@PathVariable String assetId,
			@RequestBody RmcStaffAssests newAssetDetails) {
