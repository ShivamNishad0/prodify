...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/ho/leave")
public class HoStaffLeavesController {

	@Autowired
	private HoStaffLeavesService leaveService;

	@PostMapping("/apply")
	public ResponseEntity<String> createLeave(@RequestBody HoStaffLeaves leave) {
	...
	public ResponseEntity<List<HoStaffLeaves>> getStaffLeaves(@PathVariable Long staffId) {
		List<HoStaffLeaves> leaves = leaveService.staffLeaves(staffId);
