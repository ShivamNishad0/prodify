...
@RestController
@RequestMapping("/api/spshrm/dmc/id")
@CrossOrigin
public class DmcIDController {

	@Autowired
	private DmcIDCardServices idService;
	@Autowired
	...
	public List<DmcIDCard> getAllActiveCards() {
	...
	public ResponseEntity<?> getIDCardByEmpNo(@PathVariable String empNo) {
		...
		DmcIDCard idCard = idService.findByEmpNo(decoder(empNo));
		if (idCard != null) {
	...
	public List<DmcIDCard> getAllInActiveCards() {
	...
	public ResponseEntity<?> getIDCardByEmpNo(@PathVariable List<Long> staffid) {
		...
		List<DmcIDCard> idCard = idService.idByStaffsIds(staffid);
		if (idCard != null) {
			return new ResponseEntity<>(idCard, HttpStatus.OK);
		} else {
			String result = "NO ACTIVE ID-CARD FOUND";
