...
@RestController
@RequestMapping("/api/spshrm/ho/id")
@CrossOrigin
public class HoIDController {

	@Autowired
	private HoIDCardServices idService;
	@Autowired
	...
	public List<HoIDCard> getAllActiveCards() {
	...
	public ResponseEntity<?> getIDCardByEmpNo(@PathVariable String empNo) {
		...
		HoIDCard idCard = idService.findByEmpNo(decoder(empNo));
		if (idCard != null) {
	...
	public List<HoIDCard> getAllInActiveCards() {
	...
	public ResponseEntity<?> getIDCardByEmpNo(@PathVariable List<Long> staffid) {
		...
		List<HoIDCard> idCard = idService.idByStaffsIds(staffid);
		if (idCard != null) {
			return new ResponseEntity<>(idCard, HttpStatus.OK);
		} else {
			String result = "NO ACTIVE ID-CARD FOUND";
