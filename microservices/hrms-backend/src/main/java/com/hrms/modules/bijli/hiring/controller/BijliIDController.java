...
@RestController
@RequestMapping("/api/spshrm/bijli/id")
@CrossOrigin
public class BijliIDController {

    @Autowired
    private BijliIDCardServices idService;
    @Autowired
    ...
    public List<BijliIDCard> getAllActiveCards() {
    ...
    public ResponseEntity<?> getIDCardByEmpNo(@PathVariable String empNo) {
        ...
        BijliIDCard bijliIdCard = idService.findByEmpNo(decoder(empNo));
        if (bijliIdCard != null) {
            return new ResponseEntity<>(bijliIdCard, HttpStatus.OK);
        } else {
    ...
    public List<BijliIDCard> getAllInActiveCards() {
    ...
    public ResponseEntity<?> getIDCardByEmpNo(@PathVariable List<Long> staffid) {
        ...
        List<BijliIDCard> bijliIdCard = idService.idByStaffsIds(staffid);
        if (bijliIdCard != null) {
            return new ResponseEntity<>(bijliIdCard, HttpStatus.OK);
        } else {
            String result = "NO ACTIVE ID-CARD FOUND";
