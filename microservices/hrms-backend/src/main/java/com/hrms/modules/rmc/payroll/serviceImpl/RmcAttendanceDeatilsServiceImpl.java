...
@Service
public class RmcAttendanceDeatilsServiceImpl implements RmcAttendanceDeatilsService {

    @Autowired
    private RmcAttendanceDeatilsRepository detailRepo;

    @Autowired
    private RmcStaffAttendanceRepository attendanceRepo;
    
    @Autowired
    private RmcSalaryStructureService salaryStructure;
    
    @Autowired
    private RmcStaffSalaryDetailsService salaryDetails;
    

    @Override
    public String createOrUpdateStaffAttendanceDetails(Long savedAttendanceId,Long staffId,String month,String year) {
        RmcStaffAttendance attendance = attendanceRepo.findById(savedAttendanceId).orElse(null);
        if (attendance == null) {
            return Result.STAFF_NOT_FOUND.toString();
        }
        RmcAttendanceDeatils foundDetails = detailRepo.findDetailsByDetails(staffId,month,year);
        if(foundDetails==null) {
        	RmcAttendanceDeatils details = new RmcAttendanceDeatils();
            ...
            RmcSalaryStructure ssid = salaryStructure.findByStaffId(staffId);
            if(ssid==null) {
            ...
            RmcAttendanceDeatils saved = detailRepo.save(details);
            if(saved!=null) {
        ...
        if(foundDetails!=null) {
             ...
             RmcAttendanceDeatils saved = detailRepo.save(foundDetails);
             if(saved!=null) {
             	salaryDetails.generateSalary(saved.getStaffId(),Integer.parseInt(saved.getYear()),saved.getMonth());
             }
    ...
    public List<RmcAttendanceDeatils> findAttendanceDetailsByEnpNo(String empNo, String year) {
        List<RmcAttendanceDeatils> details= detailRepo.findByEmpNo(empNo,year);
    ...
    public CountAndStaffDetails getDetailsOfStudent(int size, int page, String month, String year) {
        ...
        Page<RmcAttendanceDeatils> details = detailRepo.findDetailsByDetailsForView( month, year, pageable);
...
//    Helping Methods
    public Map<Attendance, Integer> countAttendance(RmcStaffAttendance staffAttendance) {
        ...
        try {
            for (int i = 1; i <= 31; i++) {
                Field field = RmcStaffAttendance.class.getDeclaredField("d" + i);
