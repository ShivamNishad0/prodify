...
@RestController
@RequestMapping("/api/spshrm/rmc/staff")
@CrossOrigin
public class RmcStaffController {

    @Autowired
    private RmcStaffServices staffService;
    ...
    public ResponseEntity<?> addStaff(@RequestBody NewStaffDTO newStaff) {
        ...
        if (result.equals(Result.CONTACT_NO_ALREADY_EXIST.toString())
                || result.equals(Result.EMP_NO_ALREADY_EXIST.toString())) {
...
//    }
    ...
    public ResponseEntity<?> getStaffByTempEmp(@RequestParam String tempEmp, @RequestParam String verified, @RequestParam String type) {
    ...
    public ResponseEntity<?> getdeactivatedStaffByEmpNo(@RequestParam String empNo) {
        RmcRemovedStaffs details = staffService.deactivationDetails(empNo);
        if (details == null) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
