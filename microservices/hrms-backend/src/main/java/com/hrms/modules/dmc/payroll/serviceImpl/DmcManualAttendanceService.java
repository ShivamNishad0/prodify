...
@Slf4j
@Service
public class DmcManualAttendanceService {


    @Autowired
    private DmcHolidayRepo holidayRepo;
    @Autowired
    ...
    @Autowired
    private DmcStaffSalaryDetailsRepository salaryDetails;
    @Autowired
    private DmcAttendanceDeatilsService detailService;
    @Autowired
    ...
    public String markAttendance(List<ManualAttendaceDTO> dto) {
        try {
            for (ManualAttendaceDTO data : dto) {
                ...
                DmcStaffAttendance existingAttendance = attendanceRepo.findByDetails(staff.getStaffId(), data.getMonthName(), data.getYear());
                if (existingAttendance == null) {
                    createNewAttendance(new DmcStaffAttendance(), staff, staffAttendanceDTO);
                } else {
    ...
    private String createNewAttendance(DmcStaffAttendance newAttendance, DmcStaff staff, StaffAttendanceDTO staffAt) {
        // Create a fresh instance for the current attendance record
        DmcStaffAttendance attendanceRecord = new DmcStaffAttendance();
        ...
        DmcStaffAttendance savedAttendance = attendanceRepo.save(attendanceRecord);

        if (savedAttendance != null) {
            DmcStaffSalaryDetails foundData = salaryDetails.findBydetails(
    ...
    private String updateExistingAttendance(DmcStaffAttendance found, StaffAttendanceDTO staffAt) {
        ...
        for (AttendanceDTO attendanceDTO : staffAt.getAttendance()) {
            DmcStaff staff = staffRepo.findStaffByTemp_emp(staffAt.getEmpNo());
            setDayAttendance(found, attendanceDTO, staffAt.getYear(), staffAt.getMonthName(), staff.getStaffId());
        }

        DmcStaffAttendance savedAttendance = attendanceRepo.save(found);
        if (savedAttendance != null) {
            DmcStaffSalaryDetails foundData = salaryDetails.findBydetails(savedAttendance.getStaffId(), savedAttendance.getMonthName(), savedAttendance.getYear().toString());
            String status = (foundData==null||foundData.getStatus() == null )? "N/A" : foundData.getStatus().toString();
            if (status.equals( "N/A")||status.equals(Status.UNVERIFIED.toString())) {
    ...
    public CountAndDetails getDataToMarkAttendance(String month, String year, String empNo, String[] areaId, Pageable pageable) {
        ...
        Page<DmcSalaryStructure> resultPage = fetchSalaryStructures(area, empNo, pageable);
        long count = resultPage.getTotalElements();

        // Process salary structures
        for (DmcSalaryStructure bss : resultPage) {
            DmcStaff found = staffRepo.findStaffByTemp_emp(bss.getEmpNo());
            if (found != null) {
    ...
    private Page<DmcSalaryStructure> fetchSalaryStructures(Long[] area, String empNo, Pageable pageable) {
    ...
    private ManualAttendaceDTO createAttendanceDTO(DmcSalaryStructure bss, DmcStaff found, String month, String year) {
        ...
        DmcAttendanceDeatils details = attendanceDeatilsRepo.findAttendanceDetails(empNo, month, year);
        dto.setPresent(calculateDaysPresent(details));

        return dto;
    }

    private String calculateDaysPresent(DmcAttendanceDeatils details) {
        if (details == null) {
            return "00";
        }
...
//    ---------------😊😊😊😊HELPING METHODS😊😊😊😊-----------
    ...
    private void setDayAttendance(DmcStaffAttendance newAttendance, AttendanceDTO attendanceDTO, int year, String monthName, Long staffId) {
    ...
    private Attendance checkAndApplyLeave(Date currentDate, Long staffId, Attendance defaultStatus) {
        DmcStaffLeaves leave = isAppliedForLeave(currentDate, staffId);
        DmcHoliday foundHoliday = holidayRepo.findByholidayStart(currentDate);
        if (leave != null) {
    ...
    private  DmcStaffLeaves isAppliedForLeave(Date fromDate, Long staffId) {
        DmcStaffLeaves leaves = leaveRepo.findByFromDate(fromDate, staffId);
        return leaves;
    }
