...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/bijli/assets")
public class BijliStaffAssestController {
	@Autowired
	private BijliStaffAssestService assetService;

	@PostMapping("/new-asset")
	public ResponseEntity<String> createNewAsset(@RequestBody BijliAssest asset) {
	...
	@PutMapping("/{assetId}")
	public ResponseEntity<String> updateAsset(@PathVariable Long assetId, @RequestBody BijliAssest asset) {
	...
	@GetMapping("/{assetId}")
	public ResponseEntity<BijliAssest> getByAssetId(@PathVariable Long assetId) {
		BijliAssest foundAsset = assetService.getByassetId(assetId);
		if (foundAsset != null) {
	...
	public ResponseEntity<List<BijliAssest>> getAllAssets() {
		List<BijliAssest> foundAssets = assetService.getAllAsset();
