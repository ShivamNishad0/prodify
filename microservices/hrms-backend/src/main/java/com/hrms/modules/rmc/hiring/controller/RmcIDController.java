...
@RestController
@RequestMapping("/api/spshrm/rmc/id")
@CrossOrigin
public class RmcIDController {

    @Autowired
    private RmcIDCardServices idService;
    @Autowired
    ...
    public List<RmcIDCard> getAllActiveCards() {
    ...
    public ResponseEntity<?> getIDCardByEmpNo(@PathVariable String empNo) {
        ...
        RmcIDCard idCard = idService.findByEmpNo(decoder(empNo));
        if (idCard != null) {
    ...
    public List<RmcIDCard> getAllInActiveCards() {
    ...
    public ResponseEntity<?> getIDCardByEmpNo(@PathVariable List<Long> staffid) {
        ...
        List<RmcIDCard> idCard = idService.idByStaffsIds(staffid);
        if (idCard != null) {
            return new ResponseEntity<>(idCard, HttpStatus.OK);
        } else {
            String result = "NO ACTIVE ID-CARD FOUND";
