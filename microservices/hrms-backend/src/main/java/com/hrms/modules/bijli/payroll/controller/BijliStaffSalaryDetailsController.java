...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/bijli/payroll")
public class BijliStaffSalaryDetailsController {
    @Autowired
    private BijliStaffSalaryDetailsService salaryService;
    ...
    @GetMapping("/details")
    public ResponseEntity<BijliStaffSalaryDetails> findSalaryDetails(@RequestParam Long staffId, @RequestParam String month,
                                                                     @RequestParam String year) {
        BijliStaffSalaryDetails foundSalary = salaryService.findSalaryDetails(staffId, month, year);
        if (foundSalary != null) {
    ...
    public ResponseEntity<CountAndSalaryDetailsByZone> getAllSalaries(
            ...
            @RequestParam(value = "desigId", required = false) Long desigId,
            @RequestParam(value = "req") String req,
            @RequestParam(value = "area_id", required = false) String[] area_id,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        CountAndSalaryDetailsByZone result = salaryService.allSalariesList(zoneId, month, year, type, req, desigId, area_id, page, size);
        return new ResponseEntity<>(result, HttpStatus.OK);
    }
    ...
    @GetMapping("/target/complete-salary-list")
    public ResponseEntity<CountAndSalaryDetailsByZone> getAllSalariesForTarget(
            @RequestParam("month") String month,
            @RequestParam("year") String year,
            @RequestParam(value = "type", required = false) String type,
            @RequestParam(value = "area_id", required = false) String[] area_id,
            @RequestParam(value = "report_for", required = false) String report_for
    ) {
        CountAndSalaryDetailsByZone result = salaryService.completeSalariesListForTargetBased(month, year, type, area_id, report_for);
        return new ResponseEntity<>(result, HttpStatus.OK);
    }
    ...
    public ResponseEntity<List<?>> getStaffDetails(
        ...
        List<SalaryReportDTO> details = salaryService.getAllSalaryByDetails(month, zoneId, area_id, year, type, req, desigId);
    ...
    public ResponseEntity<?> getAnnualSalary(@RequestParam("empNo") String empNo,
                                             @RequestParam("zoneId") Long zoneId,
                                             @RequestParam("year") String year) {
        List<BijliStaffSalaryDetails> result = salaryService.getAnualResultOfEmp(empNo, zoneId, year);
    ...
    public ResponseEntity<?> getStaffPrevDetails(
        ...
        BijliStaffSalaryDetails details = salaryService.findPrevAmt(emp_no, month, year);
        return ResponseEntity.ok(details);
    }
}
