...
@RestController
@RequestMapping("/api/spshrm/bijli/staff")
@CrossOrigin
public class BijliStaffController {

    @Autowired
    private BijliStaffServices staffService;
...
//    }
    ...
    @GetMapping("/staff-list/all/{zone_id}")
    public ResponseEntity<?> getAllStaff(@PathVariable("zone_id") Long zoneId,
                                         @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "20") int size) {
        List<ResponseStaffDTO> staffList = staffService.allStaffsByZone(size, page, zoneId);
        return ResponseEntity.ok(staffList);
    }

    @GetMapping("/staff-list-area/all")
    public ResponseEntity<?> getAllStaffBySubDivision(
                                                      @RequestParam("areaId") List<String> areaId,
													  @RequestParam("type") String type,
                                                      @RequestParam String verified,
    ...
    @GetMapping("/staff-list-area/complete")
    public ResponseEntity<?> getAllStaffBySubDivision(
            @RequestParam("areaId") List<String> areaId,
            @RequestParam("verified") String verified,
            @RequestParam("type") String type) {
        GetStaffAndStaffCountDTO staffs = staffService.allStaffsByZoneAndAreaSubDivision( areaId, type,verified);
        if (staffs != null) {
    ...
    public ResponseEntity<?> getStaffByTempEmp(@RequestParam String empNo) {
        BijliRemovedStaffs details = staffService.deactivationDetails(empNo);
        if (details == null) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
