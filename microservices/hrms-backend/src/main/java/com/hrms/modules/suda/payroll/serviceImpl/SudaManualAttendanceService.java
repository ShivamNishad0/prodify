...
public class SudaManualAttendanceService {

    @Autowired
    private SudaHolidayRepo holidayRepo;
    @Autowired
    ...
    @Autowired
    private SudaStaffSalaryDetailsRepository salaryDetails;
    @Autowired
    private SudaAttendanceDeatilsService detailService;
    @Autowired
    ...
    public String markAttendance(List<ManualAttendaceDTO> dto) {
        try {
            for (ManualAttendaceDTO data : dto) {
                ...
                SudaStaffAttendance existingAttendance = attendanceRepo.findByDetails(staff.getStaffId(), data.getMonthName(), data.getYear());
                if (existingAttendance == null) {
                    createNewAttendance(new SudaStaffAttendance(), staff, staffAttendanceDTO);
                } else {
    ...
    private String createNewAttendance(SudaStaffAttendance newAttendance, SudaStaff staff, StaffAttendanceDTO staffAt) {
        // Create a fresh instance for the current attendance record
        SudaStaffAttendance attendanceRecord = new SudaStaffAttendance();
        ...
        SudaStaffAttendance savedAttendance = attendanceRepo.save(attendanceRecord);

        if (savedAttendance != null) {
            SudaStaffSalaryDetails foundData = salaryDetails.findBydetails(
    ...
    private String updateExistingAttendance(SudaStaffAttendance found, StaffAttendanceDTO staffAt) {
        ...
        for (AttendanceDTO attendanceDTO : staffAt.getAttendance()) {
            SudaStaff staff = staffRepo.findStaffByTemp_emp(staffAt.getEmpNo());
            setDayAttendance(found, attendanceDTO, staffAt.getYear(), staffAt.getMonthName(), staff.getStaffId());
        }

        SudaStaffAttendance savedAttendance = attendanceRepo.save(found);
        if (savedAttendance != null) {
            SudaStaffSalaryDetails foundData = salaryDetails.findBydetails(savedAttendance.getStaffId(), savedAttendance.getMonthName(), savedAttendance.getYear().toString());
            String status = (foundData==null||foundData.getStatus() == null )? "N/A" : foundData.getStatus().toString();
            if (status.equals( "N/A")||status.equals(Status.UNVERIFIED.toString())) {
    ...
    public CountAndDetails getDataToMarkAttendance(String month, String year, String empNo, String[] areaId, Pageable pageable) {
        ...
        Page<SudaSalaryStructure> resultPage = fetchSalaryStructures(area, empNo, pageable);
        long count = resultPage.getTotalElements();

        // Process salary structures
        for (SudaSalaryStructure bss : resultPage) {
            SudaStaff found = staffRepo.findStaffByTemp_emp(bss.getEmpNo());
            if (found != null) {
    ...
    private Page<SudaSalaryStructure> fetchSalaryStructures(Long[] area, String empNo, Pageable pageable) {
    ...
    private ManualAttendaceDTO createAttendanceDTO(SudaSalaryStructure bss, SudaStaff found, String month, String year) {
        ...
        SudaAttendanceDeatils details = attendanceDeatilsRepo.findAttendanceDetails(empNo, month, year);
        dto.setPresent(calculateDaysPresent(details));

        return dto;
    }

    private String calculateDaysPresent(SudaAttendanceDeatils details) {
        if (details == null) {
            return "00";
        }
...
//    ---------------😊😊😊😊HELPING METHODS😊😊😊😊-----------
    ...
    private void setDayAttendance(SudaStaffAttendance newAttendance, AttendanceDTO attendanceDTO, int year, String monthName, Long staffId) {
    ...
    private Attendance checkAndApplyLeave(Date currentDate, Long staffId, Attendance defaultStatus) {
        SudaStaffLeaves leave = isAppliedForLeave(currentDate, staffId);
        SudaHoliday foundHoliday = holidayRepo.findByholidayStart(currentDate);
        if (leave != null) {
    ...
    private  SudaStaffLeaves isAppliedForLeave(Date fromDate, Long staffId) {
        SudaStaffLeaves leaves = leaveRepo.findByFromDate(fromDate, staffId);
        return leaves;
    }
