...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/bijli/salary")
public class BijliSalaryStructureController {

	  @Autowired
	    private BijliSalaryStructureService salaryService;
	  
	  @PostMapping("/new")
	    public ResponseEntity<String> newSalaryStructure(@RequestBody BijliSalaryStructure salary) {
	    ...
	    public ResponseEntity<String> updateSalary(@PathVariable Long salaryId, @RequestBody BijliSalaryStructure salary) {
	        String result = salaryService.updateSalary(salaryId, salary);
	        return ResponseEntity.ok(result);
	    }

	    @GetMapping("/by-staff/{salaryId}")
	    public ResponseEntity<BijliSalaryStructure> findByStaffId(@PathVariable Long salaryId) {
	        BijliSalaryStructure salary = salaryService.findByStaffId(salaryId);
	        return ResponseEntity.ok(salary);
	    }
	    
	    @GetMapping("/by-staff-data/{staffId}")
	    public ResponseEntity<BijliSalaryStructure> findBySatffId(@PathVariable Long staffId) {
	        BijliSalaryStructure salary = salaryService.findByStaffId(staffId);
	        if(salary!=null) {	        	
	        	return ResponseEntity.ok(salary);
	        }
	    ...
	    @GetMapping("/all")
	    public ResponseEntity<List<BijliSalaryStructure>> getAllSalary() {
	        List<BijliSalaryStructure> salaries = salaryService.getAllSalary();
