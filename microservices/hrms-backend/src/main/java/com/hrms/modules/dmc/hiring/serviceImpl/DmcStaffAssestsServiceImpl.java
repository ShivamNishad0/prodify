...
import com.hrms.modules.dmc.hiring.repository.DmcStaffAssestsRepo;
...
@Service
public class DmcStaffAssestsServiceImpl implements DmcStaffAssestsService {

	@Autowired
	private DmcStaffAssestsRepo assetRepo;
	@Autowired
	...
	public String createAssetChalan(NewAssetDTO assets) {
	    ...
	    DmcStaffAssests newAsset = new DmcStaffAssests();
	    ...
	    if (assets.getStaffId() != null) {
	        Optional<DmcStaff> optionalStaff = staffRepo.findById(assets.getStaffId());
	        return optionalStaff.map(staff -> {
	            staff.setIsAssetAlloted(Status.TRUE);
	            staffRepo.save(staff);
	            DmcStaffAssests savedAsset = assetRepo.save(newAsset);
	...
	public String updateStaffAsset(Long id, DmcStaffAssests newAssetDetails) {
		DmcStaffAssests existingStaff = assetRepo.findById(id).get();
		if(existingStaff!=null) {
	        ...
	        DmcStaffAssests saved=assetRepo.save(existingStaff);
