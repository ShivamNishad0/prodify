...
@RestController
@RequestMapping("/api/spshrm/dmc/staff")
@CrossOrigin
public class DmcStaffController {

    @Autowired
    private DmcStaffServices staffService;
...
//    }
    ...
    public ResponseEntity<?> getAllStaffBySubDivision(
            @RequestParam("areaId") String areaId, @RequestParam("type") String type, @RequestParam("verified") String verified, @RequestParam(defaultValue = "0") int page,
    ...
    public ResponseEntity<?> getStaffByTempEmp(@RequestParam String empNo) {
        DmcRemovedStaffs details = staffService.deactivationDetails(empNo);
        if (details == null) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
