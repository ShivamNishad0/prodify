...
@Slf4j
@Service
public class BhilaiManualAttendanceService {
    @Autowired
    private BhilaiHolidayRepo holidayRepo;
    @Autowired
    private BhilaiStaffLeavesRepository leaveRepo;
    @Autowired
    private BhilaiStaffRepo staffRepo;
    @Autowired
    private BhilaiStaffAttendanceRepository attendanceRepo;
    @Autowired
    private BhilaiStaffSalaryDetailsRepository salaryDetails;
    @Autowired
    private BhilaiAttendanceDeatilsService detailService;
    @Autowired
    ...
    public String markAttendance(List<ManualAttendaceDTO> dto) {
        try {
            for (ManualAttendaceDTO data : dto) {
                ...
                BhilaiStaff staff = staffRepo.findStaffByTemp_emp(data.getEmpNo());
                if (staff == null) {
                    return "Staff not found for EmpNo=" + data.getEmpNo();
                }

                // Check if attendance exists for the given month and year
                BhilaiStaffAttendance existingAttendance = attendanceRepo.findByDetails(staff.getStaffId(), data.getMonthName(), data.getYear());
                if (existingAttendance == null) {
                    createNewAttendance(new BhilaiStaffAttendance(), staff, staffAttendanceDTO);
                } else {
    ...
    private String createNewAttendance(BhilaiStaffAttendance newAttendance, BhilaiStaff staff, StaffAttendanceDTO staffAt) {
        // Create a fresh instance for the current attendance record
        BhilaiStaffAttendance attendanceRecord = new BhilaiStaffAttendance();
        ...
        BhilaiStaffAttendance savedAttendance = attendanceRepo.save(attendanceRecord);

        if (savedAttendance != null) {
            BhilaiStaffSalaryDetails foundData = salaryDetails.findBydetails(
    ...
    private String updateExistingAttendance(BhilaiStaffAttendance found, StaffAttendanceDTO staffAt) {
        ...
        for (AttendanceDTO attendanceDTO : staffAt.getAttendance()) {
            BhilaiStaff staff = staffRepo.findStaffByTemp_emp(staffAt.getEmpNo());
            setDayAttendance(found, attendanceDTO, staffAt.getYear(), staffAt.getMonthName(), staff.getStaffId());
        }

        BhilaiStaffAttendance savedAttendance = attendanceRepo.save(found);
        if (savedAttendance != null) {
            BhilaiStaffSalaryDetails foundData = salaryDetails.findBydetails(savedAttendance.getStaffId(), savedAttendance.getMonthName(), savedAttendance.getYear().toString());
            String status = (foundData==null||foundData.getStatus() == null )? "N/A" : foundData.getStatus().toString();
            if (status.equals( "N/A")||status.equals(Status.UNVERIFIED.toString())) {
    ...
    public CountAndDetails getDataToMarkAttendance(String month, String year, String empNo, String[] areaId, Pageable pageable) {
        ...
        Page<BhilaiSalaryStructure> resultPage = fetchSalaryStructures(area, empNo, pageable);
        long count = resultPage.getTotalElements();

        // Process salary structures
        for (BhilaiSalaryStructure bss : resultPage) {
            BhilaiStaff found = staffRepo.findStaffByTemp_emp(bss.getEmpNo());
            if (found != null) {
    ...
    private Page<BhilaiSalaryStructure> fetchSalaryStructures(Long[] area, String empNo, Pageable pageable) {
    ...
    private ManualAttendaceDTO createAttendanceDTO(BhilaiSalaryStructure bss, BhilaiStaff found, String month, String year) {
        ...
        BhilaiAttendanceDeatils details = attendanceDeatilsRepo.findAttendanceDetails(empNo, month, year);
        dto.setPresent(calculateDaysPresent(details));

        return dto;
    }

    private String calculateDaysPresent(BhilaiAttendanceDeatils details) {
        if (details == null) {
            return "00";
        }
...
//    ---------------😊😊😊😊HELPING METHODS😊😊😊😊-----------
    ...
    private void setDayAttendance(BhilaiStaffAttendance newAttendance, AttendanceDTO attendanceDTO, int year, String monthName, Long staffId) {
    ...
    private Attendance checkAndApplyLeave(Date currentDate, Long staffId, Attendance defaultStatus) {
        BhilaiStaffLeaves leave = isAppliedForLeave(currentDate, staffId);
        BhilaiHoliday foundHoliday = holidayRepo.findByholidayStart(currentDate);
        if (leave != null) {
    ...
    private  BhilaiStaffLeaves isAppliedForLeave(Date fromDate, Long staffId) {
        BhilaiStaffLeaves leaves = leaveRepo.findByFromDate(fromDate, staffId);
        return leaves;
    }
