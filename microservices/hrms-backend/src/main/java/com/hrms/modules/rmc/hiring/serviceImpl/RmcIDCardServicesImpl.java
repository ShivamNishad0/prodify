...
@Service
public class RmcIDCardServicesImpl implements RmcIDCardServices {

    @Autowired
    private RmcIDCardRepo idRepo;
    @Autowired
    ...
    public String approveAndGenerateId(Long staffId, Long userId) {
        ...
        if (optionalStaff.isPresent()) {
            RmcStaff staff = optionalStaff.get();
            ...
            RmcStaff savedStaff = staffRepo.save(staff);

            if (savedStaff != null) {
                RmcStaff s = optionalStaff.get();
                RmcIDCard oldCard = idRepo.findByEmpNo(s.getEmpNo());
                if (oldCard != null) {
                    RmcIDCard card = new RmcIDCard();
                    ...
                    RmcStaffArea area = areaRepo.findByStaff(staffId).get(0);
    ...
    public String generateIdOnly(Long staffId, Long userId) {
        RmcStaff staff = staffRepo.findById(staffId).orElse(null);
        if (staff == null) {
    ...
    private boolean isValidStaff(RmcStaff staff) {
    ...
    public String generateAndSaveIdCard(RmcStaff staff, Long userId) {
        // Create a new IDCard
        RmcIDCard card = new RmcIDCard();
    ...
    public RmcIDCard findByEmpNo(String empNo) {
        RmcIDCard foundId = idRepo.findByEmpNo(empNo);
        return foundId;
    }
    ...
    public String increaseCount(List<Long> cardId) {
        for (Long id : cardId) {
            RmcIDCard card = idRepo.findById(id).get();
    ...
    public String updateIdDetails(Long staffId, Long cardId) {
        ...
        Optional<RmcStaff> staffOpt = staffRepo.findById(staffId);
        Optional<RmcIDCard> cardOpt = idRepo.findById(cardId);

        // Check if both objects are present
        if (staffOpt.isPresent() && cardOpt.isPresent()) {
            RmcStaff staff = staffOpt.get();
            // Check if the staff ID is different and if a record with the new staff ID already exists

            Optional<RmcIDCard> existingCard = idRepo.findBystaff_id(staffId);
            if (existingCard.isPresent()) {
                // Update the ID card details
                RmcIDCard foundCard = existingCard.get();
    ...
    public List<RmcIDCard> idByStaffsIds(List<Long> ids) {
        ...
        List<RmcIDCard> foundIDCards = idRepo.findByStaffIds(ids);
