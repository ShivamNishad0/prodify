...
import com.hrms.modules.suda.hiring.repository.SudaRemovedStaffsRepo;
...
@Service
public class SudaStaffSalaryDetailsServiceImpl implements SudaStaffSalaryDetailsService {
    ...
    @Autowired
    private SudaStaffSalaryServicesForTarget salarySerive;
    @Autowired
    private SudaSalaryEarnedService salaryEarned;
    @Autowired
    ...
    public String generateSalary(Long StaffId, int year, String month) {
        ...
        Integer yearof = year;
        SudaAttendanceDeatils details = detailsRepo.findByDetails(StaffId, month, yearof.toString());
        SudaSalaryStructure structure = salaryStructureRepo.findById(details.getSalaryId()).get();
...
//        foundSalary.setDa(da);
    ...
    public String editSalaryDetails(Long sslId, SalaryDetails_EDIT_DTO details, Long editedBy) {
        SudaStaffSalaryDetails found = salaryRepo.findById(sslId).get();
...
//		found.setGross(details.getGross());
        ...
        SudaStaffSalaryDetails saved = salaryRepo.save(found);
        return saved != null ? Result.SUCCESS.toString() : Result.WENT_WRONG.toString();
    ...
    public SudaStaffSalaryDetails findSalaryDetails(Long StaffId, String month, String year) {
        SudaStaffSalaryDetails foundSalary = salaryRepo.findBydetails(StaffId, month, year);
    ...
    public String verifySalary(List<Long> salaryDetId, Long userId) {
        for (Long salaryDetailId : salaryDetId) {
            SudaStaffSalaryDetails foundSalary = salaryRepo.findById(salaryDetailId).get();
            foundSalary.setVerifiedBy(userId);
            foundSalary.setStatus(Status.VERIFIED);
            SudaStaffSalaryDetails savedSalary = salaryRepo.save(foundSalary);
            if (savedSalary != null) {
    ...
    public CountAndSalaryDetailsByZone allSalariesList(Long zoneId, String month, String year, String req, String[] area_id, int page, int size) {
        ...
        Page<SudaStaffSalaryDetails> dataPage;
        CountAndSalaryDetailsByZone result = new CountAndSalaryDetailsByZone();

        // Convert area_id to Long[] if it's not empty, otherwise set it to an array containing 0L
        Long[] areaIds = (area_id != null && area_id.length > 0)
                ? Arrays.stream(area_id).map(Long::parseLong).toArray(Long[]::new)
                : new Long[]{0L};

        // Check if the request type is salaried (SLR)
        boolean isSalariedRequest = "SLR".equalsIgnoreCase(req);

        // Check if the request has specific areaIds (not containing 0L)
        boolean hasSpecificAreaIds = areaIds.length > 0 && !Arrays.asList(areaIds).contains(0L);

        // Determine the appropriate dataPage based on the request type (salaried or not)
        if (isSalariedRequest) {
            // If salaried and has specific areaIds, use the appropriate repository method
            if (hasSpecificAreaIds) {
                dataPage = salaryRepo.findAllSalariedData(month, year, areaIds, pageable);
            } else {
                // If salaried but no specific areaIds, fetch all salaried data
                dataPage = salaryRepo.findAllSalariedData(month, year, pageable);
            }
        } else {
            // If it's not a salaried request, fetch target data
            dataPage = salaryRepo.findAllTargetData(month, year, pageable);
        }

        // Set the results and count
    ...
    @Override
    public CountAndSalaryDetailsByZone allSalariesListForTargetBased(
        ...
        if (!areaIdList.isEmpty()) {
            ...
            dataPage = salaryRepo.findAllDataForTarget(month, year, areaIdList, pageable);
        } else {
            // Fetch data without area ID filter
            dataPage = salaryRepo.findAllDataForTarget(month, year, pageable);
        }
    ...
    public CountAndSalaryDetailsByZone allSalariesListForComplete(String month, String year, String[] areaIds) {
        ...
        // Fetch data using the repository
        List<SudaStaffSalaryDetails> dataPage;
        if (!areaIdList.isEmpty()) {
            // If area IDs are provided, fetch data specific to those areas
    ...
    public CountAndSalaryDetailsByZone findByStaffEmpNo(String tempEmp, String month, String year, String type) {
        ...
        List<SudaStaffSalaryDetails> staffSalaryDetails = salaryRepo.findStaffByTempEmpOrContactNo(tempEmp, month, year, is_target_based);
    ...
    public String createDeduction(DeductionReqDTO data) {
        SudaStaffSalaryDetails staffSalary = salaryRepo.findStaffBydetails(data.getEmpNo(), data.getMonth(), data.getYear());
    ...
    public StaffAdvanceInfoDTO findAdvDetailsOfEmp(String empNo) {
        ...
        List<SudaStaffSalaryDetails> staffSalaryDetails = salaryRepo.advDetailsOfEmp(empNo);
        ...
        if (!staffSalaryDetails.isEmpty()) {
            SudaStaffSalaryDetails staffSalary = staffSalaryDetails.get(0);
        ...
        for (SudaStaffSalaryDetails staffSalary : staffSalaryDetails) {
    ...
    public List<SalaryReportDTO> getAllSalaryByDetails(String month, Long zoneId, String year, String[] area_id, String req) {
        ...
        if (isSalariedRequest && hasSpecificAreaIds) {
            details = mapSalaryReportDetails(salaryRepo.findAllSalariedData(month, year, areaIds));
        } else if (!isSalariedRequest && hasSpecificAreaIds) {
            details = mapSalaryReportDetails(salaryRepo.findAllTargetData(month, year, areaIds));
        } else if (isSalariedRequest && !hasSpecificAreaIds) {
            details = mapSalaryReportDetails(salaryRepo.findAllSalariedData(month, year));
        } else {
            details = mapSalaryReportDetails(salaryRepo.findAllTargetData(month, year));
        }
    ...
    public String createAdvPaymentOfStaff(AdvancePaymentDTO data, String type) {
        ...
        if (staffSalary != null) {
            ...
            switch (type) {
                case "adv":
                    staffSalary.setAdvance(newAmount);
                    break;
                case "setl":
                    staffSalary.setSetteled_Adv_Amt(newAmount);
                    break;
                case "rent":
                    updateNetPaid(staffSalary, newAmount, staffSalary.getRent());
                    staffSalary.setRent(newAmount);
                    break;
                case "check_incentive":
                    updateNetPaid(staffSalary, newAmount, staffSalary.getCheck_incentive());
                    staffSalary.setCheck_incentive(newAmount);
                    break;
                case "other_exp":
                    updateNetPaid(staffSalary, newAmount, staffSalary.getOther_expenses());
                    staffSalary.setOther_expenses(newAmount);
                    break;
                case "prev_setl":
    ...
    private void updateNetPaid(SudaStaffSalaryDetails staffSalary, BigDecimal newAmount, BigDecimal previousAmount) {
    ...
    public List<SudaStaffSalaryDetails> getAnualResultOfEmp(String empNo, Long zoneId, String year) {
        List<SudaStaffSalaryDetails> foundData = salaryRepo.detailsAnnually(empNo, year);
    ...
    public SumOfDistributionDTO findTotalMoneyDistribution(String month, String year, String type) {
        ...
        List<SudaStaffSalaryDetails> found = salaryRepo.allDetailsForMonth(month, year);
        List<SudaStaffSalaryDetails> filterData = filterResult(found, type);
        ...
        // Calculate totals
        for (SudaStaffSalaryDetails details : filterData) {
    ...
    public CountAndSalaryDetailsByZone allRemovedStaffSalary(int page, int size) {
        ...
        List<SudaRemovedStaffs> staffs = removeStaffRepo.findAll();
        List<SudaStaffSalaryDetails> data = staffs.stream()
        ...
        List<SudaStaffSalaryDetails> paginatedData = data.subList(start, end);
    ...
    public CountAndSalaryDetailsByZone removedStaffSalary(int page, int size, String empNo) {
        ...
        List<SudaRemovedStaffs> staffs = removeStaffRepo.removedStaffDetails(empNo);
        List<SudaStaffSalaryDetails> data = staffs.stream()
                .map(s -> salaryRepo.findStaffBydetails(s.getEmpNo(), s.getMonth(), s.getYear()))
                .filter(Objects::nonNull)
                .collect(Collectors.toList());
    ...
    public Map<String, BigDecimal> otherAllowancesDetails(String emp_no, String month, String year) {
        SudaStaffSalaryDetails details = salaryRepo.findBydetails(emp_no, month, year);
        Map<String, BigDecimal> allowance = new HashMap<>();
        if (details != null) {
            allowance.put("rent", details.getRent() == null ? BigDecimal.ZERO : details.getRent());
            allowance.put("check_incentive", details.getCheck_incentive() == null ? BigDecimal.ZERO : details.getCheck_incentive());
            allowance.put("other_exp", details.getOther_expenses() == null ? BigDecimal.ZERO : details.getOther_expenses());

        }
        if (details == null) {
            allowance.put("rent", BigDecimal.ZERO);
            allowance.put("check_incentive", BigDecimal.ZERO);
    ...
    public CountAndSalaryDetailsByZone getDataAsPerMonthAndYear(String type, String month, String year, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<SudaStaffSalaryDetails> data;
        CountAndSalaryDetailsByZone result = new CountAndSalaryDetailsByZone();
        Long count = 0l;
        if (type.equals("adv")) {
    ...
    public String salaryOnHold(String empNo, String month, String year, String type) {
        SudaStaffSalaryDetails details = salaryRepo.findStaffBydetails(empNo, month, year);
        if (details != null) {
            if (type.equals("true")) {
...
//	HELPING METHODS ------------------------------------------------------------------------------------
    ...
    public List<SudaStaffSalaryDetails> filterResult(List<SudaStaffSalaryDetails> got, String type) {
        List<SudaStaffSalaryDetails> result = new ArrayList<>();

        for (SudaStaffSalaryDetails data : got) {
            SudaStaff staff = staffRepo.findStaffByTemp_emp(data.getEmpNo());
    ...
    public CountAndDetails getDynamicColumnData(String columnName, String month, String year, int pageNumber, int pageSize) {
        String query = "SELECT staff_name, emp_no, " + columnName +
                " FROM suda.suda_staff_salary_details " +
...
//            }
    ...
    public List<TargetSalaryDetailsDTO> mapSalaryClassField(List<SudaStaffSalaryDetails> data) {
        List<TargetSalaryDetailsDTO> result = new ArrayList<>();
        for (SudaStaffSalaryDetails found : data) {
            ...
            set.setSecurityDeduction(found.getSecurityDeduction() == null ? BigDecimal.ZERO : found.getSecurityDeduction());
            set.setYear(found.getYear());
            set.setStatus(found.getStatus());
            SudaStaffTargetDetails details = targetRepo.findByDetails(found.getMonth(), found.getYear(), found.getEmpNo());
            if (details != null) {
                set.setTarget(details.getTarget() == null ? BigDecimal.ZERO : details.getTarget());
                set.setExtraTarget(details.getExtraAchived() == null ? BigDecimal.ZERO : details.getExtraAchived());
            } else {
                set.setTarget(BigDecimal.ZERO);
                set.setExtraTarget(BigDecimal.ZERO);
            }

            set.setRent(found.getRent() == null ? BigDecimal.ZERO : found.getRent());
            set.setCheck_incentive(found.getCheck_incentive() == null ? BigDecimal.ZERO : found.getCheck_incentive());
            set.setOther_expenses(found.getOther_expenses() == null ? BigDecimal.ZERO : found.getOther_expenses());
            result.add(set);
        }
        return result;
    }

    public List<SalaryReportDTO> mapSalaryReportDetails(List<SudaStaffSalaryDetails> data) {
        ...
        for (SudaStaffSalaryDetails found : data) {
            ...
            set.setNetPaid(found.getNetPaid() == null ? BigDecimal.ZERO : found.getNetPaid());
            set.setTotalNoDay(found.getTotalNoDay());
            SudaAttendanceDeatils deatils = detailsRepo.findAttendanceDetails(found.getEmpNo(), found.getMonth(), found.getYear());
            if (deatils != null) {
            ...
            SudaSalaryStructure bss = salaryStructureRepo.findByStaffId(found.getStaffId());
            ...
            SudaStaff bs = staffRepo.findById(found.getStaffId()).get();
