...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/ho/assets")
public class HoStaffAssestController {
	@Autowired
	private HoStaffAssestService assetService;

	@PostMapping("/new-asset")
	public ResponseEntity<String> createNewAsset(@RequestBody HoAssest asset) {
	...
	@PutMapping("/{assetId}")
	public ResponseEntity<String> updateAsset(@PathVariable Long assetId, @RequestBody HoAssest asset) {
	...
	@GetMapping("/{assetId}")
	public ResponseEntity<HoAssest> getByAssetId(@PathVariable Long assetId) {
		HoAssest foundAsset = assetService.getByassetId(assetId);
		if (foundAsset != null) {
	...
	public ResponseEntity<List<HoAssest>> getAllAssets() {
		List<HoAssest> foundAssets = assetService.getAllAsset();
