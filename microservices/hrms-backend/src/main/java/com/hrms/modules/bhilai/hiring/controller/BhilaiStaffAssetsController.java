...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/bhilai/staff-assets")
public class BhilaiStaffAssetsController {
	@Autowired
	private BhilaiStaffAssestsService staffAssetsService;
	...
	public ResponseEntity<List<BhilaiStaffAssests>> findByStaff(@PathVariable String staffId) {
		List<BhilaiStaffAssests> assetsList = staffAssetsService.findByStaff(Long.parseLong(decoder(staffId)));
	...
	public ResponseEntity<String> updateStaffAsset(@PathVariable String assetId,
			@RequestBody BhilaiStaffAssests newAssetDetails) {
