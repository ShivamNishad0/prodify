...
import com.hrms.modules.suda.payroll.serviceImpl.SudaManualAttendanceService;
...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/suda/attendance")
public class SudaStaffAttendanceController {
	@Autowired
	private SudaStaffAttendanceService attendanceService;

	@Autowired
	private SudaAttendanceDeatilsService attendaceDetails;

	@Autowired
	private SudaManualAttendanceService manualAttendanceService;
	...
	public ResponseEntity<SudaStaffAttendance> getAttendanceByEmpNo(@PathVariable String empNo, @RequestParam String month, @RequestParam int year) {
		SudaStaffAttendance attendance = attendanceService.findByempNo(empNo,month,year);
		if (attendance != null) {
	...
	public ResponseEntity<?> getAttendanceDetailsByEmpNo(@PathVariable String empNo,@PathVariable String year) {
		List<SudaAttendanceDeatils> details = attendaceDetails.findAttendanceDetailsByEnpNo(empNo,year);
		if (details != null) {
