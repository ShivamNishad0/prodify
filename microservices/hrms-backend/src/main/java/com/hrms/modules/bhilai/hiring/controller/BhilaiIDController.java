...
@RestController
@RequestMapping("/api/spshrm/bhilai/id")
@CrossOrigin
public class BhilaiIDController {

	@Autowired
	private BhilaiIDCardServices idService;
	@Autowired
	...
	public List<BhilaiIDCard> getAllActiveCards() {
	...
	public ResponseEntity<?> getIDCardByEmpNo(@PathVariable String empNo) {
		...
		BhilaiIDCard bhilaiIdCard = idService.findByEmpNo(decoder(empNo));
		if (bhilaiIdCard != null) {
			return new ResponseEntity<>(bhilaiIdCard, HttpStatus.OK);
		} else {
	...
	public List<BhilaiIDCard> getAllInActiveCards() {
	...
	public ResponseEntity<?> getIDCardByEmpNo(@PathVariable List<Long> staffid) {
		...
		List<BhilaiIDCard> bhilaiIdCard = idService.idByStaffsIds(staffid);
		if (bhilaiIdCard != null) {
			return new ResponseEntity<>(bhilaiIdCard, HttpStatus.OK);
		} else {
			String result = "NO ACTIVE ID-CARD FOUND";
