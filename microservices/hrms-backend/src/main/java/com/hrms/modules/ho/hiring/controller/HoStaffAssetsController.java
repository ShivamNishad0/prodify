...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/ho/staff-assets")
public class HoStaffAssetsController {
	@Autowired
	private HoStaffAssestsService staffAssetsService;
	...
	public ResponseEntity<List<HoStaffAssests>> findByStaff(@PathVariable String staffId) {
		List<HoStaffAssests> assetsList = staffAssetsService.findByStaff(Long.parseLong(decoder(staffId)));
	...
	public ResponseEntity<String> updateStaffAsset(@PathVariable String assetId,
			@RequestBody HoStaffAssests newAssetDetails) {
