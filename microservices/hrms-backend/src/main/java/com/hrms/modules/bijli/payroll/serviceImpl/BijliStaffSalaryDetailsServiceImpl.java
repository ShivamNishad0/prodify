...
import com.hrms.modules.bijli.hiring.repository.BijliRemovedStaffsRepo;
...
@Service
public class BijliStaffSalaryDetailsServiceImpl implements BijliStaffSalaryDetailsService {
    ...
    @Autowired
    private BijliStaffSalaryServicesForRMC salarySerive;
    @Autowired
    private BijliStaffAttendanceRepository attendanceRepo;
    @Autowired
    private BijliSalaryEarnedService salaryEarned;
    @Autowired
    private BijliSalaryEarnedRepository salaryEarnedRepo;
    @Autowired
    private BijliStaffRepo staffRepo;
    @Autowired
    private BijliRemovedStaffsRepo removeStaffRepo;
    @Autowired
    ...
    public String generateSalary(Long StaffId, int year, String month) {
        System.out.println("MAKING SALARY DETAILS PLEASE WAIT" + StaffId + " month " + month);
        Integer yearof = year;
        BijliAttendanceDeatils details = detailsRepo.findByDetails(StaffId, month, yearof.toString());
        BijliSalaryStructure structure = bijliSalaryStructureRepo.findById(details.getSalaryId()).get();
        ...
        if (grossSalary.compareTo(new BigDecimal(20000)) >= 0) {
            grossSalary = grossSalary.subtract(new BigDecimal(5000)); // Assign the result back to grossSalary
        }
...
//        foundSalary.setDa(da);
    ...
    public String editSalaryDetails(Long sslId, SalaryDetails_EDIT_DTO details, Long editedBy) {
        BijliStaffSalaryDetails found = salaryRepo.findById(sslId).get();
...
//		found.setGross(details.getGross());
        ...
        BijliStaffSalaryDetails saved = salaryRepo.save(found);
        return saved != null ? Result.SUCCESS.toString() : Result.WENT_WRONG.toString();
    ...
    public BijliStaffSalaryDetails findSalaryDetails(Long StaffId, String month, String year) {
        BijliStaffSalaryDetails foundSalary = salaryRepo.findBydetails(StaffId, month, year);
    ...
    public String verifySalary(List<Long> salaryDetId, Long userId) {
        for (Long salaryDetailId : salaryDetId) {
            BijliStaffSalaryDetails foundSalary = salaryRepo.findById(salaryDetailId).get();
            foundSalary.setVerifiedBy(userId);
            foundSalary.setStatus(Status.VERIFIED);
            BijliStaffSalaryDetails savedSalary = salaryRepo.save(foundSalary);
            if (savedSalary != null) {
    ...
    public CountAndSalaryDetailsByZone allSalariesList(Long zoneId, String month, String year, String type, String req, Long desigId, String[] area_id, int page, int size) {
        ...
        Page<BijliStaffSalaryDetails> dataPage;
    ...
    public String createDeduction(DeductionReqDTO data) {
        BijliStaffSalaryDetails staffSalary = salaryRepo.findStaffBydetails(data.getEmpNo(), data.getMonth(), data.getYear());
    ...
    public CountAndSalaryDetailsByZone allSalariesListForTargetBased(Long zoneId, String month, String year, String type, String[] area_id, int page, int size) {
        ...
        String status = (type == null || type.isEmpty()) ? Status.UNVERIFIED.toString() : type;
        CountAndSalaryDetailsByZone result = new CountAndSalaryDetailsByZone();
        // Convert area_id array to Long[]
        Long[] areaIds = (area_id != null && area_id.length > 0)
                ? Arrays.stream(area_id).map(Long::parseLong).toArray(Long[]::new)
                : new Long[]{0L};

        Page<BijliStaffSalaryDetails> dataPage;
        if (areaIds.length > 0 && !Arrays.asList(areaIds).contains(0L)) {
            dataPage = salaryRepo.findAllDataForTargetWithArea(month, year, areaIds, status, pageable);
            result.setCount(salaryRepo.countByAreaId(month, year, status, areaIds));
        } else {
            dataPage = salaryRepo.findAllDataForTarget(month, year, status, pageable);
    ...
    public CountAndSalaryDetailsByZone completeSalariesListForTargetBased(String month, String year, String type, String[] area_id, String report_for) {
        String status = type == null || type.equals("") ? Status.UNVERIFIED.toString() : type;
        Long[] areaIds = (area_id != null && area_id.length > 0)
                ? Arrays.stream(area_id).map(Long::parseLong).toArray(Long[]::new)
                : new Long[]{0L};
        CountAndSalaryDetailsByZone result = new CountAndSalaryDetailsByZone();

        List<BijliStaffSalaryDetails> dataPage;
        if (areaIds.length > 0 && !Arrays.asList(areaIds).contains(0L)) {
            dataPage = salaryRepo.findCompleteDataForTargetWithArea(month, year, areaIds, status);
            if (report_for.equals("WD")) {
                result.setResults(mapSalaryReportDetails(dataPage));
            } else {
                result.setResults(mapSalaryClassField(dataPage));
            }
        } else {
            dataPage = salaryRepo.findCompleteDataForTarget(month, year, status);
            result.setResults(mapSalaryClassField(dataPage));
        }
        result.setCount(salaryRepo.foundDataOfTargetBased(month, year, status));
    ...
    public CountAndSalaryDetailsByZone findByStaffEmpNo(String tempEmp, String month, String year,String type) {
        ...
        List<BijliStaffSalaryDetails> staffSalaryDetails = salaryRepo.findStaffByTempEmpOrContactNo(tempEmp, month, year,is_target_based);
    ...
    public StaffAdvanceInfoDTO findAdvDetailsOfEmp(String empNo, Long zoneId) {
        ...
        List<BijliStaffSalaryDetails> staffSalaryDetails = salaryRepo.advDetailsOfEmp(empNo);
        ...
        if (!staffSalaryDetails.isEmpty()) {
            BijliStaffSalaryDetails staffSalary = staffSalaryDetails.get(0);
        ...
        for (BijliStaffSalaryDetails staffSalary : staffSalaryDetails) {
            if (staffSalary.getAdvance().compareTo(BigDecimal.ZERO) > 0 || staffSalary.getSetteled_Adv_Amt().compareTo(BigDecimal.ZERO) > 0) {
    ...
    public List<SalaryReportDTO> getAllSalaryByDetails(String month, Long zoneId, String[] area_id, String year, String type, String req, Long desigId) {
        ...
        Long[] areaIds = (area_id != null && area_id.length > 0)
                ...
                : new Long[]{0L};
        if (req.equals("SLR")) {
            if (areaIds.length > 0 && !Arrays.asList(areaIds).contains(0L) && desigId > 0) {
                details = mapSalaryReportDetails(salaryRepo.salariedEmpAreaWiseAndDesigWise(month, year, status, areaIds, desigId));
            } else if (areaIds.length > 0 && !Arrays.asList(areaIds).contains(0L)) {
                details = mapSalaryReportDetails(salaryRepo.salariedEmpAreaWise(month, year, status, areaIds));
            } else {
                details = mapSalaryReportDetails(salaryRepo.salariedEmp(month, year, status));
            }

        } else {
            details = mapSalaryReportDetails(salaryRepo.detailsByMonth(month, year, status));
        }
    ...
    public List<BijliStaffSalaryDetails> getAnualResultOfEmp(String empNo, Long zoneId, String year) {
        List<BijliStaffSalaryDetails> foundData = salaryRepo.detailsAnnually(empNo, year);
    ...
    public SumOfDistributionDTO findTotalMoneyDistribution(String month, String year, String type) {
        ...
        List<BijliStaffSalaryDetails> found = salaryRepo.allDetailsForMonth(month, year);
        List<BijliStaffSalaryDetails> filterData = filterResult(found, type);
        ...
        // Calculate totals
        for (BijliStaffSalaryDetails details : filterData) {
    ...
    public CountAndDetails searchSalaryDetails(String searchTerm, String month, String year) {
        List<BijliStaffSalaryDetails> result = salaryRepo.methodForSearchIndivisual(searchTerm, month, year);
    ...
    public CountAndSalaryDetailsByZone allRemovedStaffSalary(int page, int size) {
        ...
        List<BijliRemovedStaffs> staffs = removeStaffRepo.findAll();
        List<BijliStaffSalaryDetails> data = staffs.stream()
        ...
        List<BijliStaffSalaryDetails> paginatedData = data.subList(start, end);
    ...
    public CountAndSalaryDetailsByZone removedStaffSalary(int page, int size, String empNo) {
        ...
        List<BijliRemovedStaffs> staffs = removeStaffRepo.removedStaffDetails(empNo);
        List<BijliStaffSalaryDetails> data = staffs.stream()
                .map(s -> salaryRepo.findStaffBydetails(s.getEmpNo(), s.getMonth(), s.getYear()))
                .filter(Objects::nonNull)
                .collect(Collectors.toList());
    ...
    public CountAndSalaryDetailsByZone getDataAsPerMonthAndYear(String type, String month, String year, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<BijliStaffSalaryDetails> data;
        CountAndSalaryDetailsByZone result = new CountAndSalaryDetailsByZone();
        Long count = 0l;
        if (type.equals("adv")) {
    ...
    public String salaryOnHold(String empNo,String month,String year,String type){
        BijliStaffSalaryDetails details = salaryRepo.findStaffBydetails(empNo,month,year);
        if (details!=null){
            if (type.equals("true")){
...
//	HELPING METHODS ------------------------------------------------------------------------------------
    ...
    public List<BijliStaffSalaryDetails> filterResult(List<BijliStaffSalaryDetails> got, String type) {
        List<BijliStaffSalaryDetails> result = new ArrayList<>();

        for (BijliStaffSalaryDetails data : got) {
            System.out.println("Looking For Staff ====> " + data.getEmpNo());
            BijliStaff staff = staffRepo.findStaffByTemp_emp(data.getEmpNo());
            if (staff != null && type.equals("active") && staff.getActive().equals(Status.ACTIVE)) {
                result.add(data);
    ...
    public CountAndDetails getDynamicColumnData(String columnName, String month, String year, int pageNumber, int pageSize) {
        String query = "SELECT staff_name, emp_no, " + columnName +
                " FROM bijli.bijli_staff_salary_details " +
...
//            }
    ...
    public List<TargetSalaryDetailsDTO> mapSalaryClassField(List<BijliStaffSalaryDetails> data) {
        List<TargetSalaryDetailsDTO> result = new ArrayList<>();
        for (BijliStaffSalaryDetails found : data) {
            ...
            BijliStaffTargetDetails details = targetRepo.findByDetails(found.getMonth(), found.getYear(), found.getEmpNo());
    ...
    public List<SalaryReportDTO> mapSalaryReportDetails(List<BijliStaffSalaryDetails> data) {
        ...
        for (BijliStaffSalaryDetails found : data) {
            ...
            set.setAdditionalRemark(found.getAdditionalRemark() == null ? "N/A" : found.getAdditionalRemark());
            set.setNetPaid(found.getNetPaid() == null ? BigDecimal.ZERO : found.getNetPaid());
            BijliAttendanceDeatils deatils=detailsRepo.findAttendanceDetails(found.getEmpNo(),found.getMonth(),found.getYear());
            if(deatils!=null){
            ...
            BijliSalaryStructure bss = bijliSalaryStructureRepo.findByStaffId(found.getStaffId());
            ...
            BijliStaff bs = staffRepo.findById(found.getStaffId()).get();
