...
import com.hrms.modules.ho.hiring.repository.HoStaffQualificationRepo;
import com.hrms.modules.ho.hiring.service.HoStaffQualificationService;
...
@Service
@Slf4j
public class HoStaffQualificationServiceImpl implements HoStaffQualificationService {
	...
	public String saveQualifications(Long staffId, QualificationDTO qualifications) {
		 HoStaffQualification savedQualification;
	            // Create a new qualification if it doesn't exist
	            HoStaffQualification newQualification = new HoStaffQualification();
	...
	public String updateQualification(QualificationDTO quali) {
	        ...
	        Optional<HoStaffQualification> optionalQualification = qualiRepo.findById(quali.getQualiID());
	        HoStaffQualification updated = null;
	        if (optionalQualification.isPresent()) {
	            HoStaffQualification found = optionalQualification.get();
	            // Update the fields
	...
	public List<HoStaffQualification> getStaffQualification(Long staffId){
		List<HoStaffQualification> staffQuali=qualiRepo.findByStaffId(staffId);
