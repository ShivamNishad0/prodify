...
import com.hrms.modules.rmc.hiring.repository.RmcStaffAssestsRepo;
import com.hrms.modules.rmc.hiring.repository.RmcStaffRepo;
import com.hrms.modules.rmc.hiring.service.RmcStaffAssestsService;
...
@Service
public class RmcStaffAssestsServiceImpl implements RmcStaffAssestsService {

	@Autowired
	private RmcStaffAssestsRepo assetRepo;
	@Autowired
	...
	public String createAssetChalan(NewAssetDTO assets) {
	    ...
	    RmcStaffAssests newAsset = new RmcStaffAssests();
	    ...
	    if (assets.getStaffId() != null) {
	        Optional<RmcStaff> optionalStaff = staffRepo.findById(assets.getStaffId());
	        return optionalStaff.map(staff -> {
	            staff.setIsAssetAlloted(Status.TRUE);
	            staffRepo.save(staff);
	            RmcStaffAssests savedAsset = assetRepo.save(newAsset);
	...
	public String updateStaffAsset(Long id, RmcStaffAssests newAssetDetails) {
		RmcStaffAssests existingStaff = assetRepo.findById(id).get();
		if(existingStaff!=null) {
	        ...
	        RmcStaffAssests saved=assetRepo.save(existingStaff);
