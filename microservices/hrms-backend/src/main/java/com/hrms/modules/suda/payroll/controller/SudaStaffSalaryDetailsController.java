...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/suda/payroll")
public class SudaStaffSalaryDetailsController {
	@Autowired
	private SudaStaffSalaryDetailsService salaryService;
	...
	@GetMapping("/details")
	public ResponseEntity<SudaStaffSalaryDetails> findSalaryDetails(@RequestParam Long staffId, @RequestParam String month,
																	@RequestParam String year) {
		SudaStaffSalaryDetails foundSalary = salaryService.findSalaryDetails(staffId, month, year);
		if (foundSalary != null) {
	...
	public ResponseEntity<CountAndSalaryDetailsByZone> getAllSalaries(
	    ...
	    CountAndSalaryDetailsByZone result = salaryService.allSalariesList(zoneId,month,year,req,area_id, page, size);
	    return new ResponseEntity<>(result, HttpStatus.OK);
	}
	...
	public ResponseEntity<CountAndSalaryDetailsByZone> getAllSalariesForTarget(
			...
			@RequestParam(defaultValue = "20") int size) {
		CountAndSalaryDetailsByZone result = salaryService.allSalariesListForTargetBased(zoneId,month,year,area_id, page, size);
		return new ResponseEntity<>(result, HttpStatus.OK);
	}
	...
	public ResponseEntity<List<?>> getStaffDetails(
	    ...
	    List<SalaryReportDTO> details = salaryService.getAllSalaryByDetails(month, zoneId, year,area_id,req);
	...
	public  ResponseEntity<?> getAnnualSalary(@RequestParam("empNo")String empNo,
											  @RequestParam("zoneId")Long zoneId,
											  @RequestParam("year")String year){
		List<SudaStaffSalaryDetails> result = salaryService.getAnualResultOfEmp(empNo,zoneId,year);
	...
	public ResponseEntity<?> getStaffPrevDetails(
		...
		SudaStaffSalaryDetails details = salaryService.findPrevAmt(emp_no, month, year);
		return ResponseEntity.ok(details);
	}
