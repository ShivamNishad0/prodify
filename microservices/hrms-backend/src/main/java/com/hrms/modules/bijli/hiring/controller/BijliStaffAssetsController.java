...
@RestController
@CrossOrigin
@RequestMapping("/api/spshrm/bijli/staff-assets")
public class BijliStaffAssetsController {
	@Autowired
	private BijliStaffAssestsService staffAssetsService;
	...
	public ResponseEntity<List<BijliStaffAssests>> findByStaff(@PathVariable String staffId) {
		List<BijliStaffAssests> assetsList = staffAssetsService.findByStaff(Long.parseLong(decoder(staffId)));
	...
	public ResponseEntity<String> updateStaffAsset(@PathVariable String assetId,
			@RequestBody BijliStaffAssests newAssetDetails) {
