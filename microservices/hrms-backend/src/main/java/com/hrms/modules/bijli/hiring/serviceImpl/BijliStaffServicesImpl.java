...
import com.hrms.modules.bijli.hiring.models.*;
import com.hrms.modules.bijli.payroll.modles.*;
import com.hrms.modules.bijli.payroll.repository.*;
import com.hrms.modules.dtos.*;
import com.hrms.modules.bijli.hiring.repository.*;
import com.hrms.modules.bijli.hiring.service.BijliStaffServices;
...
@Service
public class BijliStaffServicesImpl implements BijliStaffServices {
	...
	@Autowired
	private  BijliIDCardRepo idCardRepo;
	@Autowired
	...
	@Autowired
	private BijliStaffAssestsRepo assestsRepo;
	@Autowired
	private BijliStaffAttendanceRepository attendanceRepo;
	@Autowired
	private BijliStaffLeavesRepository leavesRepo;
	@Autowired
	private BijliStaffSalaryDetailsRepository salaryDetailsRepo;
	@Autowired
	private BijliStaffTargetDetailsRepository targetDetailsRepo;
	...
	public List<ResponseStaffDTO> allStaffs(Pageable pageable) {
	    Page<BijliStaff> staffPage = staffRepo.findAllStaffs(pageable);
	    List<ResponseStaffDTO> found = new ArrayList<>();
	    for (BijliStaff s : staffPage.getContent()) {
    ...
    public List<ResponseStaffDTO> allStaffsByZone(int size, int page, Long zoneId) {
        ...
        Page<BijliStaff> staffPage = staffRepo.allStaffByZone( pageable);
        List<ResponseStaffDTO> found = new ArrayList<>();
        for (BijliStaff s : staffPage) {
            ResponseStaffDTO rsd = new ResponseStaffDTO();
            found.add(mapForGet(rsd, s));
        }	
        return found;
	...
	@Override
	public GetStaffAndStaffCountDTO allStaffsByZoneAndAreaSubDivision(int size, int page, List<String> area_id, String type, String verified) {
		Pageable pageable = PageRequest.of(page, size);
		Page<BijliStaff> staffPage;
		Long count;

		// Convert the List<String> area_id to Long[] if it's not empty
		Long[] areaIds = (area_id != null && !area_id.isEmpty())
				? area_id.stream().map(Long::parseLong).toArray(Long[]::new)
				: new Long[]{0L};

		// Determine the active status based on the provided type
		String active = "inactive".equals(type) ? "INACTIVE" : "ACTIVE";

		// Fetch the staff records based on the presence of areaId
		if (areaIds.length > 0 && !Arrays.asList(areaIds).contains(0L)) {
			staffPage = staffRepo.allStaffByZoneAndBySubDivision(List.of(areaIds), active, verified, pageable);
			count = staffRepo.countStaffByZoneAndBySubDivision(List.of(areaIds),verified);
	    } else {
	        staffPage = staffRepo.allStaffByBySubDivision( active, verified, pageable);
	...
	public GetStaffAndStaffCountDTO allStaffsByZoneAndAreaSubDivision( List<String> area_id, String type,String verified) {
		...
		// Fetch the staff records based on the presence of areaId
			...
			staffPage = staffRepo.allStaffByZoneAndBySubDivision(List.of(areaIds), active,verified);
			count = staffRepo.countStaffByZoneAndBySubDivision(List.of(areaIds), verified);
	...
	public ResponseStaffDTO staffByID(Long staffId) {
		BijliStaff staff = staffRepo.findById(staffId).get();
		if (staff != null) {
			ResponseStaffDTO rsd = new ResponseStaffDTO();
	...
	public List<ResponseStaffDTO> staffByVerifiedStatus() {
		List<BijliStaff> staff = staffRepo.findStaffUnverifiedStatus();
		List<ResponseStaffDTO> found = new ArrayList<>();
		for (BijliStaff s : staff) {
			ResponseStaffDTO rsd = new ResponseStaffDTO();
			rsd = mapForGet(rsd, s);
			found.add(rsd);
			rsd = null;
	...
	public List<ResponseStaffDTO> staffVerifiedStatus() {
		List<BijliStaff> staff = staffRepo.findStaffVerifiedStatus();
		List<ResponseStaffDTO> found = new ArrayList<>();
		for (BijliStaff s : staff) {
			ResponseStaffDTO rsd = new ResponseStaffDTO();
			rsd = mapForGet(rsd, s);
	...
	public String generateOfferLetter(Long staffId, Long userId) {
		BijliStaff s = staffRepo.findById(staffId).get();
	...
	public String verifyCharacter(Long saffID) {
		BijliStaff staff = staffRepo.findById(saffID).get();
	...
	public String deleteStaff(RemoveStaffDTO data) {
		BijliStaff  staff = staffRepo.findById(data.getStaffId()).get();
		if (staff!=null && staff.getActive().equals(Status.ACTIVE)){
			staff.setActive(Status.INACTIVE);
			staffRepo.save(staff);
			BijliRemovedStaffs staffs =new BijliRemovedStaffs();
	...
	public List<ResponseStaffDTO> tempForGetAllStaff() {
		List<BijliStaff> staff = staffRepo.findAll();
		List<ResponseStaffDTO> found = new ArrayList<>();
		for (BijliStaff s : staff) {
			ResponseStaffDTO rsd = new ResponseStaffDTO();
			rsd = mapForGet(rsd, s);
	...
	public GetStaffAndStaffCountDTO findByTempEmp(String tempEmp, String type,String verified) {
	    ...
	    List<BijliStaff> staff = staffRepo.findStaffByTempEmp(tempEmp,active,verified);
	    List<ResponseStaffDTO> found = new ArrayList<>();
	    for (BijliStaff s : staff) {
	        ResponseStaffDTO rsd = new ResponseStaffDTO();
	        rsd = mapForGet(rsd, s);
	...
	public CountAndStaffDetails findAllDeactivatedEmp(Pageable pageable) {
		Page<BijliStaff> staffPage = staffRepo.findAllStaffsDeactivated(pageable);
	...
	public  BijliRemovedStaffs deactivationDetails(String emp_no){
		BijliRemovedStaffs staff = removeStaffRepo.deactivationDetails(emp_no);
		if (staff != null) {
			return staff;
		}
		return null;
	...
	private String handleNewStaff(NewStaffDTO newStaff) {
		...
		BijliStaff staff = createOrUpdateStaff(new BijliStaff(), newStaff, "NEW");
	...
	private String handleEditStaff(Long staffId, NewStaffDTO newStaff) {
		...
		BijliStaff existingStaff = staffRepo.findById(staffId).orElse(null);
		if (existingStaff == null) {
			return Result.NOT_FOUND.toString();
		}

		// Check for account number conflicts if a new account number is provided
		if (newStaff.getAccountNumber() != null && !newStaff.getAccountNumber().isEmpty()) {
		...
		if (newStaff.getTempEmp() != null) {
			BijliStaff staffWithTempEmp = staffRepo.findStaffByTemp_emp(newStaff.getTempEmp());
	...
	private String handleAccountNumberConflict(NewStaffDTO newStaff, Long staffId) {
		...
		BijliStaff staffWithNewAccount = staffRepo.findStaffByAccount(newStaff.getAccountNumber());
	...
	private String updateEmpNoInRelatedModels(String oldEmpNo, String newEmpNo) {
		try {
			// Update empNo in BijliStaff
			BijliStaff staff = staffRepo.findStaffByTemp_emp(oldEmpNo);
			if (staff != null) {
				staff.setTempEmp(newEmpNo);
				staffRepo.save(staff);
			}

			// Update empNo in BijliIDCard
			BijliIDCard card = idCardRepo.findByEmpNo(oldEmpNo);
			if (card != null) {
				card.setEmpNo(newEmpNo);
				idCardRepo.save(card);
			}

			// Batch update for BijliAttendanceDeatils
			List<BijliAttendanceDeatils> attendanceDetails = attendanceDeatilsRepo.findByEmpNo(oldEmpNo);
			...
			// Batch update for BijliSalaryEarned
			List<BijliSalaryEarned> salaryEarned = salaryEarnedRepo.findByEmpNo(oldEmpNo);
			if (!salaryEarned.isEmpty()) {
				salaryEarned.forEach(details -> details.setEmpNo(newEmpNo));
				salaryEarnedRepo.saveAll(salaryEarned);
			}

			// Update empNo in BijliSalaryStructure
			BijliSalaryStructure salaryStructure = salaryStructureRepo.findByEmpNo(oldEmpNo);
			if (salaryStructure != null) {
				salaryStructure.setEmpNo(newEmpNo);
				salaryStructureRepo.save(salaryStructure);
			}

			// Batch update for BijliStaffAssests
			List<BijliStaffAssests> staffAssests = assestsRepo.findByEmpNo(oldEmpNo);
			if (!staffAssests.isEmpty()) {
				staffAssests.forEach(asset -> asset.setEmpNo(newEmpNo));
				assestsRepo.saveAll(staffAssests);
			}

			// Batch update for BijliStaffAttendance
			List<BijliStaffAttendance> attendances = attendanceRepo.findStaffByEmpNo(oldEmpNo);
			if (!attendances.isEmpty()) {
				attendances.forEach(details -> details.setEmpNo(newEmpNo));
				attendanceRepo.saveAll(attendances);
			}

			// Batch update for BijliStaffLeaves
			List<BijliStaffLeaves> staffLeaves = leavesRepo.findLeaveByEmp_no(oldEmpNo);
			if (!staffLeaves.isEmpty()) {
				staffLeaves.forEach(leave -> leave.setEmpNo(newEmpNo));
				leavesRepo.saveAll(staffLeaves);
			}

			// Batch update for BijliStaffSalaryDetails
			List<BijliStaffSalaryDetails> salaryDetails = salaryDetailsRepo.findStaffByEmpNo(oldEmpNo);
			...
			// Batch update for BijliStaffTargetDetails
			List<BijliStaffTargetDetails> targetDetails = targetDetailsRepo.findByEmpNo(oldEmpNo);
...
//		}
//
//		BijliStaff existingStaff = optionalStaff.get();
//		existingStaff = createOrUpdateStaff(existingStaff, newStaff, "EDIT");
//
//		return saveStaff(existingStaff);
//	}

	private BijliStaff createOrUpdateStaff(BijliStaff staff, NewStaffDTO newStaff, String action) {
		...
		if (!action.equals("EDIT")) {
			...
			BijliStaff sc = staffRepo.getLatest();
	...
	private String saveStaff(BijliStaff staff) {
		BijliStaff savedStaff = staff != null ? staffRepo.save(staff) : null;
	...
	public ResponseStaffDTO mapForGet(ResponseStaffDTO rsd, BijliStaff s) {
		...
		List<BijliStaffQualification> quali = qualiRepo.findByStaffId(s.getStaffId());
		List<QualificationDTO> qq = new ArrayList<>();
		for (BijliStaffQualification d : quali) {
		...
		List<BijliStaffExprience> exps = expRepo.findByStaff(s.getStaffId());
		List<StaffExprienceDTOS> exs = new ArrayList<>();
		for (BijliStaffExprience se : exps) {
		...
		Optional<BijliStaffArea> optionalArea = Optional.ofNullable(s.getAreaId()).flatMap(areaRepo::findById);
