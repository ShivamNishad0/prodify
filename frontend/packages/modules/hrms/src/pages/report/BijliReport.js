...
export default function Report() {
  ...
  useEffect(() => {
    ...
    areas.forEach((ele) => {
      if (ele.location && ele.location !== "DETAILED") {
        areaSet.add(ele.location);
      } else {
        areaSet.add(ele.area);
        circleSet.add(ele.circle);
        divisionSet.add(ele.division);
        subDivisionSet.add(ele.subDivision);
      }
    });
  ...
  Array.isArray(datas) &&
    ...
    datas.forEach((ele) => {
      ...
      if (ele.area?.area) {
        ...
        const subDivisionName = ele.area.subDivision;

        // Ensure counts[areaName] is initialized
        if (!counts[areaName]) {
          counts[areaName] = {
            "Total Number Of Employees": { value: [], postCounts: {} },
            // Initialize other metrics if necessary
          };
        }
        if (!counts[subDivisionName]) {
          counts[subDivisionName] = {
            "Total Number Of Employees": { value: [], postCounts: {} },
            // Initialize other metrics if necessary
          };
        }
        ...
        if (ele.desigId) {
          ...
          counts[subDivisionName]["Total Number Of Employees"].value.push(ele);

          if (post) {
            // Ensure postCounts[post] is initialized
            if (
              !counts[areaName]["Total Number Of Employees"].postCounts[post]
            ) {
              counts[areaName]["Total Number Of Employees"].postCounts[post] =
                [];
            }
            if (
              !counts[subDivisionName]["Total Number Of Employees"].postCounts[
                post
              ]
            ) {
              counts[subDivisionName]["Total Number Of Employees"].postCounts[
            ...
            counts[areaName]["Total Number Of Employees"].postCounts[post].push(
              ele
            );
            counts[subDivisionName]["Total Number Of Employees"].postCounts[
              post
        ...
        metrics.forEach((metric) => {
          // Ensure counts[areaName][metric.label] is initialized
          if (!counts[areaName][metric.label]) {
            counts[areaName][metric.label] = { value: [], postCounts: {} };
          }
          if (!counts[subDivisionName][metric.label]) {
            counts[subDivisionName][metric.label] = {
              value: [],
              postCounts: {},
            };
          }
          ...
          if (ele[metric.key] === metric.value) {
            counts[areaName][metric.label].value.push(ele);
            counts[subDivisionName][metric.label].value.push(ele);

            if (post) {
              // Ensure postCounts[post] is initialized for this metric
              if (!counts[areaName][metric.label].postCounts[post]) {
                counts[areaName][metric.label].postCounts[post] = [];
              }
              if (!counts[subDivisionName][metric.label].postCounts[post]) {
                counts[subDivisionName][metric.label].postCounts[post] = [];
              }
              counts[areaName][metric.label].postCounts[post].push(ele);
              counts[subDivisionName][metric.label].postCounts[post].push(ele);
            }
          }
        });
      }
  ...
  const areaData = metrics.flatMap((metric) =>
  ...
  const subDivisionData = metrics.flatMap((metric) =>
    area.subDivision.map((areaName) => {
      if (counts[areaName]) {
        return {
          icon: metric.icon,
          label: `${metric.label} in ${areaName}`,
  ...
  const handleOpenModal = (data, title, region) => {
    data.forEach((ele) => {
      const post = desgn.find(
        (item) => item.desigId.toString() === ele?.desigId?.toString()
      )?.desigName;
    ...
    setModalData({ title: title, data: data, region: region });
    onOpen();
  };
          ...
          <Card key={index} className="bg-gray-100 hover:shadow-lg">
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        handleOpenModal(
                          item[post.desigName],
                          `Total Number Of ${post.desigName} in ${item.region}`,
                          item.region
                        )
                      }
                    >
            ...
            <CardFooter className="justify-evenly flex">
              ...
              <Button
                ...
                className="h-7"
                onClick={() =>
                  handleOpenModal(item.value, item.label, item.region)
                }
              >
                View
              </Button>
