...
export default function FixedSalariedEmployees() {
  ...
  async function getAllempList() {
    ...
    const response = await getData(
      ...
      }/payroll/emp-salary-details-fixed?month=${
        [...selectedMonth][0]
      }&year=${selectedYear}&type=${
        isVerified ? "VERIFIED" : "UNVERIFIED"
      }&desigId=0&req=target&zoneId=${JSON.parse(zoneCookie)}${
  ...
  const resourceUrl = useMemo(() => {
    if (selectedYear.toString().length === 4 && [...selectedMonth][0]) {
      ...
      return `${baseUrl}/api/spshrm/${
        baseZone[pathName.split("/")[pathName.includes("/hrms") ? 3 : 1]]
      }/payroll/fixed/all-salary-list?zoneId=${zone || 1}&month=${
        [...selectedMonth][0]
      }&year=${selectedYear}&type=${
        isVerified ? "VERIFIED" : "UNVERIFIED"
  ...
  async function exportRTGS(sheetData) {
    ...
    const region =
      ...
      selection.circle ||
      selection.area ||
      pathName.split("/")[pathName.includes("/hrms") ? 3 : 1]?.toUpperCase();
    const workbook = new ExcelJS.Workbook();
    ...
    const addBankDataToSheet = (worksheet, data, bankFilter, totalLabel) => {
      ...
      const total = filteredData
        .reduce((sum, item) => sum + item.netPaid, 0)
        .toFixed(2);
      worksheet.addRow([
        totalLabel,
        "",
