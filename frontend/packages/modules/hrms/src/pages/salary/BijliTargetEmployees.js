...
import { bijliSalaryExcelHeaders, months } from "@/utils/constant";
...
export default function BijliTargetEmployees() {
  ...
  const [areaIdList, setAreaIdList] = useState(new Set([""]));
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedYear, selectedMonth, areaIdList, desigId, isVerified]);

  async function getAllempList() {
    const token = await getCookie("accessToken");
    const zoneCookie = await getCookie("zone");
    const response = await getData(
      `${baseUrl}/api/spshrm/bijli/payroll/emp-salary-details?month=${
        [...selectedMonth][0]
      }&year=${selectedYear}&type=${
        isVerified ? "VERIFIED" : "UNVERIFIED"
      }&desigId=0&req=target&zoneId=${JSON.parse(zoneCookie)}${`&area_id=${[
        ...areaIdList,
      ]}`}&desigId=${desigId || 0}`,
      token
    );
  ...
  const resourceUrl = useMemo(() => {
    if (selectedYear.toString().length === 4 && [...selectedMonth][0]) {
      if (search.length > 3) {
        return `${baseUrl}/api/spshrm/bijli/payroll/search-details?emp_no=${search}&month=${
          [...selectedMonth][0]
        }&year=${selectedYear}&type=TARGET&desigId=${desigId || 0}`;
      }
      return `${baseUrl}/api/spshrm/bijli/payroll/target/all-salary-list?zoneId=${
        zone || 1
      }&month=${[...selectedMonth][0]}&year=${selectedYear}&type=${
        isVerified ? "VERIFIED" : "UNVERIFIED"
      }&area_id=${[...areaIdList]}&desigId=${desigId || 0}&page=${
        page - 1
      }&size=20`;
    }
    return null;
  ...
  async function handleApproveSalary() {
    ...
    const resposne = await postData(
      `${baseUrl}/api/spshrm/bijli/payroll/verify?userId=${user.id}`,
      { salaryDetId: payload },
      token
    );
    if (resposne) {
  ...
  const exportToExcel = async (option, sheetData, area) => {
    ...
    bijliSalaryExcelHeaders[0][0] = `Nature of work: BIJLI ${
      area ? area.toUpperCase() : ""
    } ${region ? "(" + region + ")" : ""}`;

    bijliSalaryExcelHeaders[2][5] = `${[...selectedMonth][0]}-${selectedYear}`;
    ...
    const excelHeaders = JSON.parse(JSON.stringify(bijliSalaryExcelHeaders));
    ...
    if (option === "withoutDetails") {
      ...
      // --- ARREAR MONTHS LOGIC FOR WITHOUT DETAILS ---
      // 1. Collect all unique arrear months from the data
      const allArrearMonths = new Set();
      filteredOnHoldData.forEach((emp) => {
        if (emp.arrearMonths) {
          Object.keys(emp.arrearMonths).forEach((month) =>
            allArrearMonths.add(month)
          );
        }
      });
      const arrearMonthList = Array.from(allArrearMonths).sort();

      // 2. Find index of "Emp. ESI" in the header row
      const empEsiIdx = excelHeaders[4].findIndex((h) => h === "Emp. ESI");

      // 3. Insert arrear columns after "Emp. ESI" in all header rows
      arrearMonthList.forEach((month, idx) => {
        for (let i = 0; i < 4; i++) {
          excelHeaders[i].splice(empEsiIdx + 1 + idx * 2, 0, "", "");
        }
        excelHeaders[4].splice(
          empEsiIdx + 1 + idx * 2,
          0,
          `${month} PF`,
          `${month} ESI`
      ...
      // Remove "Particulars" columns from all header rows except the 5th row
      const modifiedHeaders = excelHeaders.map((row, i) => {
        if (i === 2) {
          ...
          // Remove "Particulars" columns from other rows
      ...
      worksheetData = [
        ...
        ...filteredOnHoldData.map((item, index) => {
          ...
          // Insert arrear months after Emp. ESI
          let insertAt = 9; // After empESI (index in row)
          arrearMonthList.forEach((month) => {
            const arrear = item.arrearMonths?.[month] || {};
            row.splice(insertAt + 1, 0, arrear.empPF || 0, arrear.empESI || 0);
            insertAt += 2;
          });
          ...
          row.push(
            ...
            parseFloat(item.netPaid) -
              parseFloat(item.setteled_Adv_Amt || 0) +
              parseFloat(item.prevSetldAmt || 0) +
              parseFloat(item.other || 0) || 0,
      ...
      merges = [
        {
          start: { row: 1, col: 1 },
          end: { row: 1, col: worksheetData[0].length },
        },
      ...
      // --------- ARREAR MONTHS LOGIC STARTS HERE ---------
      // 1. Collect all unique arrear months from the data
      const allArrearMonths = new Set();
      filteredOnHoldData.forEach((emp) => {
        if (emp.arrearMonths) {
      ...
      // 2. Find index of "Emp. ESI" in the header row
      const empEsiIdx = normalizedHeaders[4].findIndex((h) => h === "Emp. ESI");

      // 3. Insert arrear columns after "Emp. ESI" in all header rows
      arrearMonthList.forEach((month, idx) => {
        for (let i = 0; i < 4; i++) {
          normalizedHeaders[i].splice(empEsiIdx + 1 + idx * 2, 0, "", "");
        }
        normalizedHeaders[4].splice(
          empEsiIdx + 1 + idx * 2,
          0,
          `${month} PF`,
          `${month} ESI`
      ...
      worksheetData = [
        ...
        ...filteredOnHoldData.map((item, index) => {
          ...
          // Insert arrear months after Emp. ESI
          let insertAt = empEsiIdx + 1;
          arrearMonthList.forEach((month) => {
            const arrear = item.arrearMonths?.[month] || {};
            row.splice(insertAt, 0, arrear.empPF || 0, arrear.empESI || 0);
            insertAt += 2;
          });
      ...
      merges = [
        ...
        { start: { row: 4, col: 21 }, end: { row: 4, col: 23 } },
      ];
      sheetName = `${region.toUpperCase() ?? "_"}${
        [...selectedMonth][0]
  ...
  const generatePdfFromSheetData = (
    ...
    const natureOfWork = `Nature of work: BIJLI ${
      area ? ` ${area.toUpperCase().replaceAll("_", " ")}` : ""
    } ${region ? "(" + region + ")" : ""}`;
  ...
  async function exportRTGS(sheetData) {
    ...
    const addBankDataToSheet = (worksheet, data, bankFilter, totalLabel) => {
      ...
      const total = filteredData
        ...
        .toFixed(2);
      worksheet.addRow([totalLabel, "", "", total, "", "", "", "", ""]);
      return total;
    };
