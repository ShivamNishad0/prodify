...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
        ...
        const checkAuth = async () => {
            if (token) {
                try {
                    ...
                    // Sync cookies for legacy HRMS modules
                    const expires = new Date(Date.now() + 7 * 864e5).toUTCString();
                    document.cookie = `accessToken=${token}; expires=${expires}; path=/; SameSite=Lax;`;
                    document.cookie = `user=${encodeURIComponent(JSON.stringify(response.data.user))}; expires=${expires}; path=/; SameSite=Lax;`;
                } catch (error) {
    ...
    const login = async (email: string, password: string) => {
        try {
            ...
            // Sync cookies for legacy HRMS modules
            const expires = new Date(Date.now() + 7 * 864e5).toUTCString();
            document.cookie = `accessToken=${newToken}; expires=${expires}; path=/; SameSite=Lax;`;
            document.cookie = `user=${encodeURIComponent(JSON.stringify(userData))}; expires=${expires}; path=/; SameSite=Lax;`;
    ...
    const logout = useCallback(async () => {
        ...
        // Clear cookies for legacy HRMS modules
